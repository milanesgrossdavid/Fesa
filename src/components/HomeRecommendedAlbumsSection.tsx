...
const HomeRecommendedAlbumsSection = ({
  ...
  <View className="pt-7">
    <Text className="mb-3 px-5 text-lg font-bold text-white">Álbumes recomendados</Text>
    <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ paddingHorizontal: 20, gap: 18 }}>
      {albums.map(album => (
        <Pressable key={album.id} className="w-40" onPress={() => onOpenAlbum(album)}>
          ...
          <Text className="mt-3 text-base font-bold text-white" numberOfLines={1}>
            {album.name}
          </Text>
          <Text className="mt-1 text-sm text-[#707070]" numberOfLines={1}>
            {album.subtitle}
          </Text>
        </Pressable>
      ))}
